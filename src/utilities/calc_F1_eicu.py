...
def get_data(length, batch_number):
    train_x = np.load(os.path.join(path, "data", "lab_values", "training_batches_eicu", f"testing_x_{length}_{batch_number}.npy"),
                      allow_pickle=True)
    print(train_x.shape)
    train_y = np.load(os.path.join(path, "data", "lab_values", "training_batches_eicu", f"testing_y_{length}_{batch_number}.npy"),
                      allow_pickle=True)
...
def predict_value(seq_length_min,seq_length_max,model_name,f1_threshold, Max_batches_number):
    ...
    test_batches_names=pd.read_csv(os.path.join(path, "data", "lab_values", "training_batches_eicu", "test_batches_names.csv"))
    ...
    for length in range(seq_length_min,seq_length_max):
        ...
        for batch_number in range(1):
            print(batch_number)
            x_test, y_test, batch_size = get_data(length, batch_number)
