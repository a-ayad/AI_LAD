...
def predict_value(window_length, model_name, f1_threshold):
    ...
    print(os.getcwd())
    test_x_l = np.load(
        os.path.join(cwd, "..", '..', "data", "lab_values", "windowed_data_DVL", f"test_x_l_WL{window_length}.npy"),
        allow_pickle=True)
    test_x_v = np.load(
        os.path.join(cwd, "..", '..', "data", "lab_values", "windowed_data_DVL", f"test_x_v_WL{window_length}.npy"),
        allow_pickle=True)
    test_x_d = np.load(
        os.path.join(cwd, "..", '..', "data", "lab_values", "windowed_data_DVL", f"test_x_d_WL{window_length}.npy"),
        allow_pickle=True)
    test_y = np.load(
        os.path.join(cwd, "..", '..', "data", "lab_values", "windowed_data_DVL", f"test_y_WL{window_length}.npy"),
        allow_pickle=True)
    test_x_l = np.asarray(test_x_l).astype('float32')
    test_x_v = np.asarray(test_x_v).astype('float32')
    test_x_d = np.asarray(test_x_d).astype('float32')
    test_y = np.asarray(test_y).astype('float32')
    print(test_x_l.shape)
    print(test_x_v.shape)
    print(test_x_d.shape)
    print(test_y.shape)
    batch_size = test_x_l.shape[0]

    print(f"current batch size ={batch_size}")


    results = model.evaluate([test_x_l,test_x_v,test_x_d], test_y, batch_size=64)
