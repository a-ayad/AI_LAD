...
input_data_path_eicu = os.path.join(cwd, "..", '..', "data", "lab_values", "eICU_edited.csv")
input_data_path_mimic = os.path.join(cwd, "..", '..', "data", "lab_values", "input_w_lab_rearranged.csv")
output_data_path_mimic = os.path.join(cwd, "..", '..', "data", "lab_values", "lab_results.csv")
output_data_path_eicu = os.path.join(cwd, "..", '..', "data", "lab_values", "eICU_lab_results.csv")
...
def scale_data(data, load_scaler):
    data_ids = data.iloc[:, :2]
    data_no = data.iloc[:, 2:].copy()
    # load the scaler
    if load_scaler:
        scaler = load(open(os.path.join(cwd, 'scaler_mimic.pkl'), 'rb'))
        logging.debug("mimic scaler is loaded")
        scaled_ds = scaler.transform(data_no)
        scaled_ds = np.around(scaled_ds, decimals=2)
        ...
        print("eicu scaler is applied")
        # save the scaler
        dump(scaler, open(os.path.join(cwd, 'scaler_eicu.pkl'), 'wb'))
        scaled_ds = np.around(scaled_ds, decimals=2)
        scaled_ds = pd.DataFrame(scaled_ds, columns=data_no.columns)
        new_df = pd.concat([data_ids, scaled_ds], axis=1)
    return new_df
...
def read_data_eicu(window_length, min_window_length=4, max_window_length=20, load_scaler=True):
    # read the input and output files
    input_data_mimic = pd.read_csv(input_data_path_mimic, )
    input_data_eicu = pd.read_csv(input_data_path_eicu, )
    output_data_mimic = pd.read_csv(output_data_path_mimic, )
    output_data = pd.read_csv(output_data_path_eicu, )
    # rearrange the columns like in mimic dataset
    columns_names_input = input_data_mimic.columns.tolist()
    columns_names_output = output_data_mimic.columns.tolist()
    input_data_eicu = input_data_eicu[columns_names_input]
    output_data = output_data[columns_names_output]
    # scale the input data
    input_data_scaled = scale_data(input_data_eicu, load_scaler)
    ...
    logging.debug(f"number of unique icu stays from eICU dataset= {y}")
    ...
    for i in range(window_length + 1, max_Seq_length):
        ...
        for window_shift in range(window_length, i):
            if window_shift == window_length:
                ...
                # print("Full X window first  \n", windowed_x)
                # print("Full Y window first \n", windowed_y_one)
    ...
    '''
    stacked_input_data_path = os.path.join(cwd, "..", '..', "data", "lab_values", "windowed_data_eicu_DVL",
                                           f"stacked_input_{window_length}.csv")
    stacked_output_data_path = os.path.join(cwd, "..", '..', "data", "lab_values", "windowed_data_eicu_DVL",
    ...
    logging.debug(f"Total number of sequences from eICU with length= {window_length} equals to {no_of_sequences} \n")
...
if __name__ == "__main__":
    ...
    window_length = 6
    load_scaler = True
    # for window_length in range(4,15):
    read_data_eicu(window_length, min_window_length, max_window_length, load_scaler)
